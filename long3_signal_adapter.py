...
SCAN_RUNTIME = Path("scan_runtime.json")
...
def public_get(path: str, params: dict):
    ...
    if not r.ok or str(p.get("code")) != "00000":
        raise RuntimeError(
            f"public GET failed: HTTP={r.status_code} code={p.get('code')} msg={p.get('msg')}"
        )
...
def asset_snapshot(symbol: str) -> dict:
    try:
        ...
        current_boundary = floor_15m(now_ms)
...
def scan_started_at(fallback: str) -> str:
    try:
        row = json.loads(SCAN_RUNTIME.read_text(encoding="utf-8"))
        return str(row.get("scan_started_at") or fallback)
    except Exception:
        return fallback
...
def market_snapshot() -> dict:
    ...
    snap = {
        "btc_1h_pct": btc["ret_1h_pct"],
        "btc_4h_pct": btc["ret_4h_pct"],
        "btc_24h_pct": btc["ret_24h_pct"],
        "btc_volatility_state": None,
        "eth_1h_pct": eth["ret_1h_pct"],
        "eth_4h_pct": eth["ret_4h_pct"],
        "eth_24h_pct": eth["ret_24h_pct"],
        "eth_btc_relative_strength_24h_pct": None,
        "total3_4h_pct": None,
        "total3_24h_pct": None,
        "total3es_4h_pct": None,
        "total3es_24h_pct": None,
        "alt_positive_4h_pct": None,
        "alt_median_4h_pct": None,
        "market_regime": None,
    }
    if btc["ret_1h_pct"] is not None:
        a = abs(btc["ret_1h_pct"])
        snap["btc_volatility_state"] = (
            "HIGH" if a >= 3.0 else ("ELEVATED" if a >= 1.5 else "NORMAL")
        )
    if btc["ret_24h_pct"] is not None and eth["ret_24h_pct"] is not None:
        snap["eth_btc_relative_strength_24h_pct"] = (
            eth["ret_24h_pct"] - btc["ret_24h_pct"]
        )
...
def choose_group(rows: list[dict]) -> dict | None:
    ...
    for row in rows:
        for code in str(row.get("all_matches") or "").split("+"):
            ...
            if code:
                all_matches.add(code)

    selected_rows = [
        r for r in rows if str(r.get("strategy", "")).upper() == selected
    ]
