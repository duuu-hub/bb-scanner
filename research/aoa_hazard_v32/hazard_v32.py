...
OUT=ROOT/"research"/"aoa_hazard_v32"/"output"
...
def actual_stats(year):
    ...
    dur=g["duration_sec"]/3600.0
    return {
        "legs":int(len(g)),
        "win_rate_pct":float((g["ret_bps"]>0).mean()*100),
        "q25_duration_h":float(dur.quantile(.25)),
        "median_duration_h":float(dur.median()),
        "q75_duration_h":float(dur.quantile(.75)),
...
def sim_one(candles,fast_model,start,end,threshold,min_hold_bars,confirm_bars=2,cost_side=0.0):
    ...
    return {
        ...
        "q25_duration_h":float(l["bars"].quantile(.25)*0.25),
        "median_duration_h":float(l["bars"].median()*0.25),
        "q75_duration_h":float(l["bars"].quantile(.75)*0.25),
...
def choose_threshold(candles,fast_model,train_df):
    # Fidelity calibration only. Search thresholds / refractory periods / confirmation
    # to imitate 2020 leg count and holding-time distribution. PnL is NOT in the objective.
    flip_scores=train_df.loc[train_df["flip"]==1,"p"].dropna()
    threshold_qs=[.50,.60,.70,.75,.80,.85,.90]
    thresholds=sorted(set(float(flip_scores.quantile(q)) for q in threshold_qs))

    ep=pd.read_csv(EP)
    dt=pd.to_datetime(ep["st"],unit="s",utc=True)
    train_ep=ep[dt<TRAIN_END].copy()
    dur_bars=train_ep["duration_sec"]/900.0
    min_hold_qs=[.10,.20,.25,.33,.50]
    min_holds=sorted(set(max(1,int(math.ceil(float(dur_bars.quantile(q))))) for q in min_hold_qs))
    confirms=[1,2,3]
    ...
    for th in thresholds:
        for mh in min_holds:
            for cb in confirms:
                ...
                score=(
                    abs(math.log(max(m["legs"],1)/max(actual["legs"],1))) +
                    lr(m["median_duration_h"],actual["median_duration_h"]) +
                    0.5*lr(m["q25_duration_h"],actual["q25_duration_h"]) +
                    0.5*lr(m["q75_duration_h"],actual["q75_duration_h"])
                )
                rows.append({
                    "threshold":th,"min_hold_bars":mh,"confirm_bars":cb,
                    "fidelity_score":score,
                    "actual_legs":actual["legs"],
                    "actual_q25_h":actual["q25_duration_h"],
                    "actual_median_h":actual["median_duration_h"],
                    "actual_q75_h":actual["q75_duration_h"],
                    **m
                })
    tab=pd.DataFrame(rows).sort_values(["fidelity_score","threshold","min_hold_bars","confirm_bars"]).reset_index(drop=True)
    best=tab.iloc[0]
    return float(best["threshold"]),int(best["min_hold_bars"]),int(best["confirm_bars"]),tab
...
def main():
    ...
    th,min_hold,confirm_bars,cal=choose_threshold(candles,fast_model,tr)
    ...
    diag={
        ...
        "selected_threshold_by_2020_fidelity":th,
        "selected_min_hold_bars":min_hold,
        ...
        "note":"Threshold/min-hold/confirmation selected only to imitate 2020 leg count and holding-time distribution. PnL is excluded from fidelity objective. 2021 untouched."
    }
