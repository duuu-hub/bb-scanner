...
TFS={"15m":15,"1H":60,"4H":240,"1D":1440,"3D":4320}
...
def feats(d):
    ...
    for n,mins in TFS.items():
        ...
        ma=h.rolling(20).mean().shift(1)
        sd=h.rolling(20).std(ddof=0).shift(1)
        m=pd.Series(ma.reindex(bucket).to_numpy(),index=idx)
        s=pd.Series(sd.reindex(bucket).to_numpy(),index=idx)
        out[n+"_UP"]=(x.close>m+2*s).to_numpy()
        out[n+"_DN"]=(x.close<m-2*s).to_numpy()
    if not hasattr(feats,"_printed"):
        print("SANITY","rows",len(x),"close_finite",int(np.isfinite(x.close).sum()),
              *[(k,int(np.isfinite(v).sum()),int(v.sum())) for k,v in out.items()],flush=True)
        feats._printed=True
    return x,out

def main():
    fs=sorted(ROOT.rglob("*.parquet"))
    assert len(fs)==666
    names=[n+s for n in TFS for s in ("_UP","_DN")]
    stats={}
    def add(key,r):
    ...
    for fi,p in enumerate(fs):
        ...
        for n in names:
            ...
            for hn,k in FWD.items():
                ids2=ids[ids+k<len(close)]
                add(("single",n,"",hn),close[ids2+k]/close[ids2]-1)
        for a,b in itertools.combinations(names,2):
            if a.split("_")[0]==b.split("_")[0]:
                continue
            st=f[a]&f[b]
            ids=np.flatnonzero(st & ~np.r_[False,st[:-1]])
            for hn,k in FWD.items():
                ids2=ids[ids+k<len(close)]
                add(("pair",a,b,hn),close[ids2+k]/close[ids2]-1)
        if fi%100==0:
    ...
    assert len(o)>0 and (o.n>0).any(),"EMPTY_SIGNAL_OUTPUT: causal BB scan produced zero events"
    Path("artifacts").mkdir(exist_ok=True)
    o.to_csv("artifacts/bb_causal_rescan.csv",index=False)
    q=o[(o.horizon=="24H")&(o.n>=300)].sort_values("mean_ret_pct",ascending=False)
    print("FILES_DONE",len(fs),flush=True)
    print("TOP_24H"); print(q.head(40).to_string(index=False),flush=True)
