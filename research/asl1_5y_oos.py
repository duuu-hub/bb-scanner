...
def sim(x,idx,hh,sym,side,btcfeat):
    ...
    for i in idx:
        ...
        out.append((sym,x.dt.iloc[ei],x.dt.iloc[ex],hh,side,r,reason,float(x.ret4h.iloc[i]),float(x.rvpre.iloc[i]),float(x.rvmed.iloc[i]),bf.get("ret4h",np.nan),bf.get("ret24h",np.nan),bf.get("open",np.nan),bf.get("ma50",np.nan),bf.get("ma200",np.nan),bf.get("btc_rv24",np.nan)))
    return out
...
btcfeat=btc.set_index("ts")[["ret4h","ret24h","open","ma50","ma200","btc_rv24"]].to_dict("index")
rows=[]
...
t=pd.DataFrame(rows,columns=["symbol","entry_dt","exit_dt","hold_h","side","net_ret","reason","asset_ret4h","asset_rvpre","asset_rvmed","btc_ret4h","btc_ret24h","btc_open","btc_ma50","btc_ma200","btc_rv24"]); t.to_csv(OUT/"trades.csv",index=False)
def stat(g):
